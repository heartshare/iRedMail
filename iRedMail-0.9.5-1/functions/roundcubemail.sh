...
GRANT CREATE,SELECT,INSERT,UPDATE,DELETE,ALTER ON ${RCM_DB_NAME}.* TO "${RCM_DB_USER}"@"${HOSTNAME}" IDENTIFIED BY '${RCM_DB_PASSWD}';
...
-- GRANT INSERT,UPDATE,SELECT ON ${VMAIL_DB_NAME}.alias TO "${RCM_DB_USER}"@"${HOSTNAME}";
